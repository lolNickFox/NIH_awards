...
b_all_dup <- b %>%
  group_by(PI_1, PI_2, PI_3, PI_4, PI_5, PI_6) %>%
  filter( n() > 1 )

b_all_dup <- b_all_dup %>%
  group_by(ACTIVITY) %>% 
  mutate(DUP_COST = sum(TOTAL_COST), PERCENT_DUP_COST = (DUP_COST / SUM_COST) * 100)
...
b %>% 
  ggplot(aes(x = IC_NAME, y = TOTAL_COST)) + geom_bar(stat = "identity") + theme(axis.text.x = element_text(angle = 45, hjust = 1))
