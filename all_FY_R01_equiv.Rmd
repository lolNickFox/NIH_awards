...
p <- ARRA_NUMBERS %>% 
  filter(FY == 2009) %>% 
  ggplot(aes(x = GRANTS, y = VALUE, fill = GRANT_TYPE)) + geom_bar(stat = "identity", colour = "#454545") + fte_theme() + labs(title="Number of unique investigators receiving R01 equivalent funds\nby ARRA status in FY2009", x = "\nNumber of funded grants", y="Number of investigators\n") + geom_hline(yintercept=0, size=0.4, color="black") + coord_cartesian(ylim = c(0, 15000)) + scale_fill_manual(values = c("#4c4cff", "#c0392b"))
